...
BEGIN
    CREATE TABLE dbo.Schedules (
        ...
        CONSTRAINT FK_Schedules_Students_StudentId FOREIGN KEY (StudentId) REFERENCES Students(Id) ON DELETE SET NULL
    );
END
ELSE
BEGIN
    ...
    IF COL_LENGTH('dbo.Schedules', 'StudentId') IS NULL
    BEGIN
        ALTER TABLE dbo.Schedules
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_Schedules_Students_StudentId')
    BEGIN
        ALTER TABLE dbo.Schedules
        WITH CHECK ADD CONSTRAINT FK_Schedules_Students_StudentId
            FOREIGN KEY (StudentId) REFERENCES Students(Id)
            ON DELETE SET NULL;
    END
END
...
BEGIN
    ...
    IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Schedules_StudentId' AND object_id = OBJECT_ID('dbo.Schedules'))
        CREATE INDEX IX_Schedules_StudentId ON dbo.Schedules(StudentId);
END";
